...
function selectTier(t){
  ...
  // ... existing code for enchants ...
    
    // START NEW CODE: Load Enchant with Levels
    var enchantLevels = getFunctionFromData(itemdata, "enchant_with_levels");
    if(enchantLevels && enchantLevels.levels){
      document.getElementById("item-enchant-min").value = enchantLevels.levels.min;
      document.getElementById("item-enchant-max").value = enchantLevels.levels.max;
    } else {
      document.getElementById("item-enchant-min").value = "";
      document.getElementById("item-enchant-max").value = "";
    }

    // Load Enchant Book For Trading
    var enchantBook = getFunctionFromData(itemdata, "enchant_book_for_trading");
    document.getElementById("item-enchant-book").checked = !!enchantBook;
    // END NEW CODE
...
// ... existing code ...
  ...
  // START NEW CODE: Update Enchant with Levels
  var lvlMin = document.getElementById("item-enchant-min").value;
  var lvlMax = document.getElementById("item-enchant-max").value;
  ...
  if(lvlMin !== "" && lvlMax !== ""){
    addFunctionToData(itemdata, {
      ...
      "treasure": false,
      "levels": {
        "min": parseFloat(lvlMin),
        "max": parseFloat(lvlMax)
      }
    }, true);
  } else {
    if(getFunctionFromData(itemdata, "enchant_with_levels")){
      removeFunctionFromData(itemdata, "enchant_with_levels");
    }
  }

  // Update Enchant Book For Trading
  if(document.getElementById("item-enchant-book").checked){
    addFunctionToData(itemdata, {
    ...
    if(getFunctionFromData(itemdata, "enchant_book_for_trading")){
      removeFunctionFromData(itemdata, "enchant_book_for_trading");
    }
  }
  // END NEW CODE

  var tradeEl = currentTradeElement; // Existing line
  correctTradeData(tradedata); // Existing line
